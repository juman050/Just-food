    ...
    <tbody>
        @if($lists)
            ...
            <tr>
                <td>{{$i++}}</td>
                <td>{{$list->name}}</td>
                <td><img src="{{asset('media/allergy/'.$list->image)}}" height="60px"></td>
                <td>
                    <div class="switch">
                ...
                <td>
                    <a href="javascript:void(0);"  data-href="{{action('Backend\AllergyController@edit', [$list->id])}}"  class="btn btn-xs btn-primary edit_allergy_button"><i class="fa fa-pencil"></i></a>
                    <a href="javascript:void(0);" onclick="deleteAllergy('{!! $list->id !!}')" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></a>
                </td>
            </tr>
            @endforeach
        @else
